<form data-dialog="size=auto;reload-on-close"
      action="<?= $controller->url_for("show/updatevorlage/" . basename(parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH))) ?>"
    ...
    <label>
        <?= _("Beschreibung") ?>
        <input type="text" name="description" required="" class="size-l"
               value="<?= htmlReady($template_description) ?>">
    </label>
